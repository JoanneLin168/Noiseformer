...
class Unet(nn.Module):
    ...
    def _decode_only(self, c0, c1, c2, c3, c4, x):
        x = self.up1(x)
    ...
    def forward(self, x, task='forward'):
        if task == 'encode':
            return self._encode_only(x)
        elif task == 'decode':
            return self._decode_only(*x)
        
        c0, c1, c2, c3, c4, x = self._encode_only(x)
        x = self._decode_only(c0, c1, c2, c3, c4, x)
