...
def load_from_checkpoint_ab(folder_name, device='cuda:0', ep='latest', new_model = False):
    ...
    # Ensure all the separators in the path are the same
...
def split_into_patches(x, patch_size = 64, channels=4, numframes=16):
    patches = torch.empty([1,channels,numframes,patch_size,patch_size], device=x.device)
...
class NoiseGenerator2d3d_distributed_ablation(nn.Module):
    def __init__(self, net, unet_opts = 'noUnet', device = 'cuda:0', noise_list = 'shot_read_row'):
        ...
        # TODO: add the new noise parameters here
...
class LowlightNoiseGenerator2d3d_distributed_ablation(NoiseGenerator2d3d_distributed_ablation):
    def __init__(self, net, unet_opts = 'noUnet', device = 'cuda:0', noise_list = 'shot_read_row', light_list = 'alpha_beta_gamma'):
        super(LowlightNoiseGenerator2d3d_distributed_ablation, self).__init__(net, unet_opts, device, noise_list)
        
        self.light_list = light_list
        self.alpha = torch.nn.Parameter(torch.tensor(1.0, dtype = self.dtype, device = device), requires_grad = True)
        self.beta = torch.nn.Parameter(torch.tensor(0.0, dtype = self.dtype, device = device), requires_grad = True)
        self.gamma = torch.nn.Parameter(torch.tensor(1.0, dtype = self.dtype, device = device), requires_grad = True)
    ...
    def forward(self, x, split_into_patches = False, i0=None):
        ...
        # x = self.alpha(x**self.gamma) + self.beta
        if 'gamma' in self.light_list:
            x = x**self.gamma
        if 'alpha' in self.light_list:
            x = self.alpha*x
        if 'beta' in self.light_list:
            x = x + self.beta

        return super().forward(x, split_into_patches, i0)
